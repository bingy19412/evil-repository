...
let scores = { one: 0, two: 0, three: 0, four: 0 };
...
let onlineModeValue = 'ONLINE 1V1';
let localSlot = 0;
const stages = {
...
const players = {
	one: { x: 110, y: 280, radius: 17, speed: 205, angle: 0, cooldown: 0, dash: 0, color: colors.one, controls: ['w','a','s','d'], dashKey: ' ', networkSlot: 0 },
	two: { x: 790, y: 280, radius: 17, speed: 205, angle: Math.PI, cooldown: 0, dash: 0, color: colors.two, controls: ['arrowup','arrowleft','arrowdown','arrowright'], dashKey: 'enter', networkSlot: 1 },
	three: { x: 110, y: 150, radius: 17, speed: 205, angle: 0, cooldown: 0, dash: 0, color: '#f28bd6', controls: ['i','j','k','l'], dashKey: 'o', networkSlot: 2 },
	four: { x: 790, y: 410, radius: 17, speed: 205, angle: Math.PI, cooldown: 0, dash: 0, color: '#a78bfa', controls: ['t','f','g','h'], dashKey: 'y', networkSlot: 3 }
};
...
function resetRound() {
	...
	players.three.x = width * .16; players.three.y = height * .25; players.three.angle = 0; players.three.cooldown = 0; players.three.dash = 0; players.three.powered = false; players.three.moveSoundCooldown = 0;
	players.four.x = width * .84; players.four.y = height * .75; players.four.angle = Math.PI; players.four.cooldown = 0; players.four.dash = 0; players.four.powered = false; players.four.moveSoundCooldown = 0;
...
function activePlayerEntries() { const count = onlineActive ? (onlineModeValue === 'ONLINE 1V1' ? 2 : 4) : 2; return Object.entries(players).slice(0, count); }
function activePlayers() { return activePlayerEntries().map(([, player]) => player); }
...
function connectRelay() {
	...
	relaySocket.addEventListener('message', event => {
		...
		if (message.type === 'hosted') { onlineRole = 'host'; localSlot = message.slot; onlineModeValue = message.mode; lobbyCode.value = message.code; lobbyStatus.textContent = `LOBBY ${message.code} CREATED // WAITING FOR ${message.capacity} PLAYERS`; }
		if (message.type === 'joined') { onlineRole = 'client'; localSlot = message.slot; onlineModeValue = message.mode; lobbyStatus.textContent = `JOINED ${message.code} // ${message.players}/${message.capacity} PLAYERS CONNECTED`; }
		if (message.type === 'player-joined') lobbyStatus.textContent = `PLAYER JOINED // ${message.players} PLAYERS CONNECTED`;
		if (message.type === 'match-ready') { lobbyStatus.textContent = `MATCH READY // STARTING ${message.mode}`; startOnlineMatch(onlineRole); }
...
function handleNetworkMessage(message) {
	if (message.type === 'input' && onlineRole === 'host') return remoteInputs[message.slot] = new Set(message.keys);
	if (message.type === 'state' && onlineRole === 'client') networkState = message;
}
function inputPressed(player, key) {
	if (onlineActive && onlineRole === 'host' && player.networkSlot !== 0) return (remoteInputs[player.networkSlot] || new Set()).has(key);
	return pressed(key);
}
...
function applyNetworkState(state) {
	if (!state) return;
	for (const side of Object.keys(players)) { const source = state.players[side]; if (source) Object.assign(players[side], source); }
...
function sendNetworkInput(dt) {
	networkSendTimer -= dt;
	if (networkSendTimer <= 0) { networkSendTimer = .05; relaySend({ type: 'input', slot: localSlot, keys: [...keys] }); }
}
function broadcastNetworkState() {
	if (onlineRole !== 'host') return;
	relaySend({ type: 'state', players, bolts, cores, scores, roundTime, winner, matchWon: winner ? scores[winner] >= 5 : false, winnerMessage: winner ? winnerSubtext.textContent : '' });
}
function update(dt) {
	if (onlineActive && onlineRole === 'client') { sendNetworkInput(dt); applyNetworkState(networkState); return; }
	if (winner) return;
	roundTime = Math.max(0, roundTime - dt); for (const [side, player] of activePlayerEntries()) updatePlayer(player, side, dt);
	const pulseState = getPulseState(); if (pulseState.active) for (const [side, player] of activePlayerEntries()) if (Math.abs(player.x - pulseState.x) < 22) { const other = activePlayerEntries().find(([candidate]) => candidate !== side)?.[0] || side; scores[other]++; endRound(other, 'The pulse caught them out.'); break; }
	for (let index = cores.length - 1; index >= 0; index--) { const core = cores[index]; core.pulse += dt * 3; for (const [side, player] of activePlayerEntries()) if (Math.hypot(core.x-player.x, core.y-player.y) < 28) { player.powered = true; burst(core.x, core.y, colors.gold, 15); playSound('collect'); cores.splice(index, 1); setTimeout(spawnCore, 850); arenaMessage.textContent = `${side === 'one' ? 'PLAYER ONE' : `PLAYER ${side.toUpperCase()}`} CHARGED // KEEP MOVING TO FIRE`; updateChargeStatus(); break; } }
	for (let index = bolts.length - 1; index >= 0; index--) { const bolt = bolts[index]; bolt.x += bolt.vx * dt; bolt.y += bolt.vy * dt; bolt.life -= dt; if (bolt.life <= 0 || bolt.x < 0 || bolt.x > width || bolt.y < 30 || bolt.y > height || collidesWithObstacle(bolt.x, bolt.y, bolt.radius)) { bolts.splice(index, 1); continue; } const target = activePlayerEntries().find(([side, player]) => side !== bolt.owner && Math.hypot(bolt.x-player.x, bolt.y-player.y) < player.radius + bolt.radius); if (target) { bolts.splice(index, 1); burst(target[1].x, target[1].y, target[1].color, 30); playSound('hit'); scores[bolt.owner]++; endRound(bolt.owner, bolt.powered ? 'Charged bolt landed. Brilliant timing.' : 'Direct hit. The arena is yours.'); break; } }
	...
	if (roundTime <= 0) endRound(activePlayerEntries().sort(([, first], [, second]) => second.x - first.x)[0][0], 'Time expired. Bold positioning wins.');
...
function playerLabel(side) { return `PLAYER ${({ one: 'ONE', two: 'TWO', three: 'THREE', four: 'FOUR' })[side]}`; }
function endRound(side, message) { if (winner) return; winner = side; scoreOne.textContent = scores.one; scoreTwo.textContent = scores.two; const matchWon = scores[side] >= 5; winnerText.textContent = matchWon ? `${playerLabel(side)} TAKES THE MATCH` : `${playerLabel(side)} WINS`; winnerSubtext.textContent = matchWon ? 'First to five. The rift is yours.' : message; playButton.innerHTML = matchWon ? 'NEW MATCH <span>↻</span>' : 'PLAY NEXT ROUND <span>→</span>'; playSound('win'); roundOver.classList.remove('hidden'); }
function draw() {
	...
	activePlayers().forEach(drawPlayer); sparks.forEach(s => { context.globalAlpha = Math.max(0, s.life * 2); context.fillStyle = s.color; context.fillRect(s.x,s.y,3,3); context.globalAlpha = 1; });
}
...
document.querySelector('#resetButton').addEventListener('click', () => { enableAudio(); if (onlineActive && onlineRole !== 'host') return; scores = { one: 0, two: 0, three: 0, four: 0 }; roundNumber = 1; resize(); resetRound(); }); playButton.addEventListener('click', () => { enableAudio(); if (onlineActive && onlineRole !== 'host') return; if (Object.values(scores).some(score => score >= 5)) scores = { one: 0, two: 0, three: 0, four: 0 }; else roundNumber++; resize(); resetRound(); }); stageSelect.addEventListener('change', event => { enableAudio(); if (onlineActive && onlineRole !== 'host') { stageSelect.value = stage; return; } stage = event.target.value; scores = { one: 0, two: 0, three: 0, four: 0 }; roundNumber = 1; resize(); resetRound(); });
function startOnlineMatch(role) { onlineRole = role; onlineActive = true; networkState = null; remoteInputs = {}; mainMenu.classList.add('hidden'); gameShell.classList.remove('hidden'); resize(); resetRound(); lobbyStatus.textContent = `${onlineModeValue} // HOST IS AUTHORITATIVE`; }
